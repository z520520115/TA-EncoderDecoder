...
def train(model, optimizer, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        loss = loss_function(pred.transpose(1, 2), labels) # encoder + Transformer
        # loss = loss_function(pred, labels) # DCNN
        loss.backward()
        accu_loss += loss.item()
...
def evaluate(model, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        loss = loss_function(pred.transpose(1, 2), labels)  # Change dimension to match (N, C, L)
...
def main(args):
    ...
    model = nn.DataParallel(DCNN().to(device), device_ids=[0, 1, 2, 3])
    ...
    for epoch in range(args.epochs):
        ...
        torch.save(model.state_dict(), "././model/DCNN.pkl")
